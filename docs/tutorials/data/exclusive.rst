...
When generating a Monte Carlo sample, a decay card (usual extension:
:code:`dec`) can be used to overwrite the decay of certain particles. This
allows you to generate a Monte Carlo sample that only contains events with the
signal topology which you are studying.
...
called :code:`DECAY.dec` in the :code:`BesEvtGen` package will be used. This
file essentially follows the PDG listings. In addition, definitions of
particles (including their physical widths) can be found in the file
:code:`pdt.table`. Both files are located here (in the case of BOSS 7.0.4):
