...
The `lxslc` server has a very outdated version of Python. If you do want to use Python
...
`alias condaenv="source <path_to_script>/conda_env.sh"` in your `.bashrc`.

Next, just run `conda activate tensorflow-gpu` and you have `python3`, `ipython` and
even `import tensorflow` available! (At the time of writing, TensorFlow is version
1.13.1 though.)

Unfortunately, you don't have the rights to `conda create` new environments. To see
