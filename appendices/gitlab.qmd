...
2. Add the SSH key to the `ssh-agent` and create a corresponding _public key_ with the commands:
   ...
   ```
3. Now, obtain the corresponding public key using:
   ```bash
   cat ~/.ssh/id_rsa.pub
   ```
   and copy all the text you see there (from `ssh-rsa` to `@ihep.ac.cn`).
4. Go to [code.ihep.ac.cn/profile/keys](https://code.ihep.ac.cn/profile/keys), click "Add SSH Key", paste the code there, and "Add key".
5. That's it!
...
As a test, you can now create a new repository on the server. Just click ["New project"](https://code.ihep.ac.cn/projects/new) and follow the instructions. This is a nice way to start, as you will be immediately shown instructions on how to configure `git` locally (such as setting the user name).

:::{.callout-note}
As an alternative, you can forward SSH credentials from your local machine. This can be done by adding the line

```{.default filename="~/.ssh/config"}
ForwardAgent yes
```

to your SSH config file for the IHEP server. This can be used to allow the usage of external servers e.g. GitHub as well.
:::
...
### Step 3: Add the files in the directory {#step-3}
...
### Step 4: Commit the changes {#step-4}
...
### Step 5: Check the status of the repository {#step-5}
...
### Step 6: Configure the Git repository {#step-6}

If you have applied for an account and [added an SSH key](#pushing-existing-code-to-a-new-repository), you can push this new repository to [code.ihep.ac.cn](https://code.ihep.ac.cn). If you haven't already done so, set the user name and email address for this repository:
...
### Step 7: Create the repository on the server {#step-7}
...
Go to [code.ihep.ac.cn](https://code.ihep.ac.cn) and click "New repository". Use `TestRepo` as the " _Project_ name", then click "Customize repository name?" to ensure that the name of the repository is `TestRepo` as well. (If you don't, it will be named `testrepo`, while **the _repository_ name should match the name of your directory**. As you see, the default option for a new repository is private, so only you can see it. <!-- cspell:ignore testrepo -->

### Step 8: Push your the first commit {#step-8}
...
You can now edit and add files and then go through steps [3 (add)](#step-3), [4 (commit)](#step-4), [5 (status)](#step-5), and [8 (push)](#step-8) to track your files.
